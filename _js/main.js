...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (
    window.location.pathname === '/services/kitchen-therapy.html' ||
    window.location.pathname === '/services/cookery.html'
  ) {
    const slider = tns({
    ...
    elsFilter.forEach((filter) => {
      filter.addEventListener('change', (e) => {
        ...
        if (value == '') {
          ...
          indexesToShow =
            filter.name == 'dates' ? dateIndexes[value] : tagIndexes[value];
        }
        ...
        //Reset other filter
        let otherFilter =
          elsFilter[0] == e.target ? elsFilter[1] : elsFilter[0];
        console.log(e);
    ...
    elSendBtn.addEventListener('click', async (e) => {
      ...
      if (errorNames.length === 0) {
        ...
        if (res.ok) {
          ...
          spawnModal(
            `<span>Oops! There seems to have been a problem sending your email on our end. Please try again :)</span>`
          );
        }
      } else {
    ...
    const elsExpandTestimonial = document.querySelectorAll(
      '.js-testimonial-more'
    );
